...
def changeJokers(handResult):
    for count, i in enumerate(handResult):
        numberOfJ = 0
        if i.split(",")[1] == "J":
            numberOfJ = int(i.split(",")[0])
            if numberOfJ != 5:
                print(f"in {handResult} are {numberOfJ} Js")
                del handResult[count]
                handResult[0] = (str((int(handResult[0].split(",")[0]) + int(numberOfJ))) + "," + handResult[0].split(",")[1])
    return handResult       
...
for line in Lines:
    ...
    handResult = changeJokers(handResult)
    value = calcValueForHandresult(handResult)
    #create new List with for each hand a list containing: MatchValue | hand | bet | HandValue
    gameItem = []
...
# sort upcending on matchValue and second decending on HandValue 
